...
DevsetSentences* devsetSentences = new DevsetSentences();
    ...
    for (auto it = dataset_sentences.begin(); it != dataset_sentences.end(); ++it) {
       ...
       // for(auto j = wordUpdates.begin(); j != wordUpdates.end(); ++j) {
        ...
        /* if (counter % 1 == 0 || counter == dataset_sentences.size()) {
            ...
            sentimentMatrix = subtractTwoMatrices(mi    nibarchRNNParam->getSentimentWeightsMatrix(), multiplyMatrixByScalar(minibarchRNNParam->getSentimentWeightsMatrix(), minibatchLearningRate));
